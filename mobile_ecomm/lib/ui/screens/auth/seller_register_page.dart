...
/// Multi-step registration for seller accounts
...
class _SellerRegisterPageState extends ConsumerState<SellerRegisterPage> {
  ...
  String? _validateStep3() {
    ...
    if (_dti == null) {
      return 'Please upload DTI Certificate';
    }
    if (_birTin == null) {
      return 'Please upload BIR TIN';
    }
    if (_businessPermit == null) {
      return 'Please upload Business Permit';
    }
    if (_validId == null) {
      return 'Please upload Valid ID';
    }
    return null;
  }
  ...
  Future<void> _handleSubmit() async {
    ...
    final success = await ref.read(authProvider.notifier).registerSeller(
  ...
  Widget _buildVerificationSection(ThemeData theme) {
    return Column(
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'Check your email for the verification code to activate your seller account',
          textAlign: TextAlign.center,
          style: theme.textTheme.bodyMedium?.copyWith(
            color: theme.colorScheme.onSurfaceVariant,
          ),
        ...
        const SizedBox(height: 8),
        Text(
          'Your shop application will be reviewed by admin after email verification',
          textAlign: TextAlign.center,
        ...
        if (_emailVerified) ...[
          ...
          HeroButton(
            onPressed: () {
              context.go('${AppRouter.login}?role=seller');
            },
            text: 'Go to Login',
          ),
        ],
    ...
    return Column(
      ...
      children: [
        ...
        TextFormField(
          ...
          decoration: const InputDecoration(
            labelText: 'Tagline',
            hintText: 'A short catchy phrase',
            border: OutlineInputBorder(),
          ),
        ),
        const SizedBox(height: 16),
        ...
        Text(
          'Select all categories that describe your products',
        ...
        FileUploader(
          label: 'Upload Logo',
          value: _logo,
          onUpload: (file) => setState(() => _logo = file),
        ),
      ],
    ).animate(effects: AppAnimations.fadeIn(delay: 0.1));
  }
  ...
  Widget _buildStep3Form() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        _buildDocumentUploader('DTI Certificate', _dti, (f) => setState(() => _dti = f)),
        const SizedBox(height: 12),
        _buildDocumentUploader('BIR TIN', _birTin, (f) => setState(() => _birTin = f)),
        const SizedBox(height: 12),
        _buildDocumentUploader('Business Permit', _businessPermit, (f) => setState(() => _businessPermit = f)),
        const SizedBox(height: 12),
        _buildDocumentUploader('Valid ID', _validId, (f) => setState(() => _validId = f)),
      ],
    ).animate(effects: AppAnimations.fadeIn(delay: 0.1));
  }
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            TextButton(
              onPressed: () => context.go('${AppRouter.login}?role=seller'),
