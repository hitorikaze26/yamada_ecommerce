...
class SellerProductsState {
  ...
  int get activeProducts => products.where((p) => p['visibility'] == true).length;
  int get inactiveProducts => products.where((p) => p['visibility'] == false).length;
  int get totalSold => products.fold<int>(0, (sum, p) => sum + (p['sold'] as int? ?? 0));
...
class SellerProductsNotifier extends StateNotifier<SellerProductsState> {
  ...
  Future<void> fetchProducts({bool silent = false}) async {
    ...
    developer.log('Starting to fetch seller products', name: 'SellerProductsNotifier');

    try {
      final products = await SellerProductsApi.getSellerProducts();
      developer.log('Successfully fetched ${products.length} products', name: 'SellerProductsNotifier');
      state = state.copyWith(
        products: products,
        isLoading: false,
        clearError: true,
      ...
      developer.log('Error fetching seller products: $e', name: 'SellerProductsNotifier');
      state = state.copyWith(
        isLoading: false,
  ...
  Future<bool> deleteProduct(String productId) async {
    try {
      ...
      final updatedProducts = state.products.where((p) => p['id'].toString() != productId).toList();
      state = state.copyWith(products: updatedProducts);
      
      return true;
    } catch (e) {
      developer.log('Error deleting product: $e', name: 'SellerProductsNotifier');
...
final sellerProductsProvider = StateNotifierProvider<SellerProductsNotifier, SellerProductsState>((ref) {
